...
import { Loader2, LocateFixed, Search, X, SlidersHorizontal } from "lucide-react";
import { PROVINCES, SPECIALTIES, type Province, type Specialty } from "../types";
...
export default function Sidebar({
  totalCount, nearbyCount, province, specialty,
  onProvinceChange, onSpecialtyChange,
  ...
  const handleTextSearch = useCallback(async () => {
    ...
    setGeoLoading(false);
    if (!result) {
      setGeoError('Location not found. Try "Kathmandu" or "Pokhara".');
      return;
    }
  ...
  const handleBrowserLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        onLocationChange({ lat: pos.coords.latitude, lon: pos.coords.longitude });
        setResolvedPlace("Your current location");
        setGeoLoading(false);
      },
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="rounded-lg bg-green-50 border border-green-100 px-3 py-2 text-center">
              <p className="text-lg font-bold text-green-700">{nearbyCount}</p>
              <p className="text-xs text-green-600">Nearby</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {geoError && (
              <p className="text-xs text-red-600 bg-red-50 border border-red-100 rounded-md px-2.5 py-1.5">{geoError}</p>
            )}
