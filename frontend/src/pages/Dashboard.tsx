...
import {
  ...
  useNearbyFacilities, useUpdateFacility,
...
export default function Dashboard() {
  ...
  const { data: specialtyFacilities = [] } = useFacilitiesBySpecialty(specialty);
  const { data: nearbyFacilities = [] } = useNearbyFacilities(
    userLocation?.lat ?? null, userLocation?.lon ?? null, radius
  );
  const updateFacility = useUpdateFacility();

  // Decide which set of facilities to show on the map
  const mapFacilities: Facility[] = useMemo(() => {
    if (specialty !== "All") return specialtyFacilities;
    if (province !== "All") return provinceFacilities;
    return allFacilities;
  }, [specialty, province, specialtyFacilities, provinceFacilities, allFacilities]);
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-background">
      ...
      <main className="flex-1 relative overflow-hidden">
        ...
        {/* Loading overlay */}
        {isLoading && mapFacilities.length === 0 && (
        ...
        {/* Selected facility detail + admin panel */}
        {selectedFacility && (
            ...
            <div className="bg-card/97 backdrop-blur-md rounded-2xl border border-border shadow-xl overflow-hidden">
              ...
              <div className="px-4 pt-4 pb-3 flex items-start gap-3">
                <div
                  ...
                  style={{ background: `${selectedFacility.hospital_type === "Teaching" ? "#7c3aed" : selectedFacility.hospital_type === "Government" ? "#2563eb" : selectedFacility.hospital_type === "Community" ? "#0891b2" : selectedFacility.hospital_type === "Specialty" ? "#d97706" : "#dc2626"}18` }}
                >
                  <MapPin className="h-4 w-4" style={{ color: selectedFacility.hospital_type === "Teaching" ? "#7c3aed" : selectedFacility.hospital_type === "Government" ? "#2563eb" : selectedFacility.hospital_type === "Community" ? "#0891b2" : selectedFacility.hospital_type === "Specialty" ? "#d97706" : "#dc2626" }} />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{selectedFacility.latitude.toFixed(5)}°N, {selectedFacility.longitude.toFixed(5)}°E</p>
                </div>
                <button onClick={() => setSelectedFacility(null)} className="text-muted-foreground hover:text-foreground shrink-0">
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="px-4 pb-3 flex flex-wrap gap-1.5">
                ...
                {selectedFacility.specialties !== "General" &&
                  selectedFacility.specialties.split(",").map((s) => (
                    <span key={s} className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-blue-50 text-blue-700">
                      {s}
                    </span>
                  ))}
                <span className={cn(
